...
public class BaseChartAttrs {
    ...
    public int yAxisZeroLineType;//default == 0, from the left of the view to the right, else value == 1 from
    public boolean enableYAxisLineDash;//控制 Y轴用虚线
    public boolean enableYAxisZero;// 控制是否显示 Y轴中的 0 刻度线
    public boolean enableYAxisGridLine;// 控制是否显示 y轴对应的横轴网格线
    public boolean enableStartYAxisLabel;//控制是否显示 Y轴右刻度
    public boolean enableEndYAxisLabel;// 控制是否显示 Y轴左刻度
    ...
    public boolean enableXAxisBg;//允许 画底部X坐标处的背景颜色。
    ...
    public boolean enableEndDayXAxis;//当天最后一个柱子是否显示XAxis
    ...
    public float xAxisBgRadius;//X轴背景底部圆角半径
    public int xAxisLabelPosition;//X轴坐标的位置，XAxis POSITION_CENTER = 0(default), POSITION_LEFT = 1, POSITION_RIGHT= 2;
    public int xFirstLinePosition;//X GrandLine 大周期线 POSITION_LEFT = 1, POSITION_RIGHT= 2; 默认靠左
    public int xAxisLabelFont;//x 周坐标字体
    ...
    public boolean dynamicAdjustItemFillWidth;

    public float highLightBigTextSize;

    public float highLightSmallTextSize;
