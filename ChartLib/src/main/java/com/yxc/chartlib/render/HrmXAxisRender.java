...
final public class HrmXAxisRender<T extends BaseChartAttrs> {
    ...
    public HrmXAxisRender(T barChartAttrs) {
    ...
    final public void drawVerticalLine(Canvas canvas, RecyclerView parent, XAxis xAxis) {
        ...
        List<RecyclerBarEntry> entries = mAdapter.getEntries();
    ...
    private boolean isNearEntrySecondType(List<? extends RecyclerBarEntry> entries, XAxis xAxis, int itemWidth, int adapterPosition) {
        if (adapterPosition == 0) {
            return false;
        }
        RecyclerBarEntry barEntryNext;
        for (int nearPosition = adapterPosition - 1; nearPosition > 0; nearPosition--) {
            ...
            ValueFormatter valueFormatter = (ValueFormatter) xAxis.getValueFormatter();
            String xAxisLabel = valueFormatter.getBarLabel(barEntryNext);
